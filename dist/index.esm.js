...
import * as SDK$1 from 'sdk-latest';
...
class LinearPool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            if (linearPoolPairData.pairType === PairTypes$1.MainTokenToBpt) {
                ...
                // Limit is amount of BPT in for pool balance of tokenOut
                const limit = SDK$1.LinearMath._calcBptInPerWrappedOut(
                    bnum(poolPairData.balanceOut.toString()),
    ...
    _exactWrappedTokenInForMainOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcMainOutPerWrappedIn(
                amtScaled,
    ...
    _exactMainTokenInForWrappedOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcWrappedOutPerMainIn(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                {
                    fee: bnum(poolPairData.swapFee.toString()),
    ...
    _exactMainTokenInForBPTOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = SDK$1.LinearMath._calcBptOutPerMainIn(
                    amtScaled,
                    bnum(poolPairData.mainBalanceScaled.toString()),
    ...
    _exactBPTInForMainTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = SDK$1.LinearMath._calcMainOutPerBptIn(
                    amtScaled,
                    bnum(poolPairData.mainBalanceScaled.toString()),
                    bnum(poolPairData.wrappedBalanceScaled.toString()),
                    bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _exactWrappedTokenInForBPTOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcBptOutPerWrappedIn(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                bnum(poolPairData.wrappedBalanceScaled.toString()),
                bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _exactBPTInForWrappedTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcWrappedOutPerBptIn(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                bnum(poolPairData.wrappedBalanceScaled.toString()),
                bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _wrappedTokenInForExactMainOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcWrappedInPerMainOut(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                {
                    fee: bnum(poolPairData.swapFee.toString()),
    ...
    _mainTokenInForExactWrappedOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcMainInPerWrappedOut(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                {
                    fee: bnum(poolPairData.swapFee.toString()),
    ...
    _mainTokenInForExactBPTOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // in = main
                // out = BPT
                const amt = SDK$1.LinearMath._calcMainInPerBptOut(
                    amtScaled,
                    bnum(poolPairData.mainBalanceScaled.toString()),
                    bnum(poolPairData.wrappedBalanceScaled.toString()),
                    bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _BPTInForExactMainTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = SDK$1.LinearMath._calcBptInPerMainOut(
                    amtScaled,
                    bnum(poolPairData.mainBalanceScaled.toString()),
                    bnum(poolPairData.wrappedBalanceScaled.toString()),
                    bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _wrappedTokenInForExactBPTOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcWrappedInPerBptOut(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                bnum(poolPairData.wrappedBalanceScaled.toString()),
                bnum(poolPairData.virtualBptSupply.toString()),
    ...
    _BPTInForExactWrappedTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK$1.LinearMath._calcBptInPerWrappedOut(
                amtScaled,
                bnum(poolPairData.mainBalanceScaled.toString()),
                bnum(poolPairData.wrappedBalanceScaled.toString()),
                bnum(poolPairData.virtualBptSupply.toString()),
